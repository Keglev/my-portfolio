...
describe('RepoDocs', () => {
  ...
  it('renders placeholder docs in German and filters summary-only repos out', async () => {
    ...
    global.fetch.mockResolvedValue({
      ...
      json: async () => ([
        ...
        },
        {
          name: 'ast-repo',
          _ast: {
            children: [{ type: 'heading', children: [{ value: 'API documentation' }] }],
          },
        },
        {
          name: 'text-repo',
    ...
    expect(screen.getByText('ast-repo')).toBeInTheDocument();
    expect(screen.getByText('text-repo')).toBeInTheDocument();
    expect(screen.queryByText('summary-only')).toBeNull();
  });

  it('renders structured docs links and converts raw GitHub URLs', async () => {
    ...
    global.fetch.mockResolvedValue({
      ...
      json: async () => ([
        ...
        },
        {
          name: 'docs-api',
          docs: {
            apiDocumentation: { link: 'https://docs.example/api' },
          },
        },
        {
          name: 'docs-doc',
          docs: {
            documentation: { link: 'https://docs.example/doc' },
          },
        },
      ]),
    });
  ...
  it('covers the remaining title-selection and docs-detection branches', async () => {
    useTranslation.mockReturnValue({ t: (k) => k, i18n: { language: 'en' } });
    ...
    global.fetch.mockResolvedValue({
      ...
      json: async () => ([
        ...
        { name: 'docs-title-en', docsTitle: 'Custom Title', docsLink: 'https://example.com/en' },
        {
          name: 'repo-api-title',
          repoDocs: {
            apiDocumentation: { title: 'API Title', link: 'https://example.com/api' },
          },
        },
        {
          name: 'repo-arch-title',
          repoDocs: {
            architectureOverview: { title: 'Architecture Title', link: 'https://example.com/arch' },
          },
        },
        {
          name: 'docs-api-title',
          docs: {
            apiDocumentation: { title: 'Docs API Title', link: 'https://example.com/docs-api' },
          },
        },
        {
          name: 'docs-doc-title',
          docs: {
            documentation: { title: 'Docs Title', link: 'https://example.com/docs' },
          },
        },
        {
          name: 'ast-title',
          _ast: {
            children: [{ type: 'heading', children: [{ value: 'Architecture docs' }] }],
          },
        },
        {
          name: 'text-title',
