...
| Script | Purpose |
|--------|---------|
| `scripts/docs/build_docs.js` | Converts every `docs/*.md` file to `docs/*.html` using the `docs/templates/page.html` template; generates a sidebar TOC from h2/h3 headings; wraps Mermaid code blocks in `.mermaid-wrapper` divs; copies `hub.html` to `docs/index.html` |
